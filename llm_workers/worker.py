import sys
from typing import Optional, Any, Union, List, Iterator, Dict, AsyncIterator, Callable

from langchain.globals import set_debug, get_debug, get_verbose
...
class LlmWorker(Runnable[str, List[BaseMessage]]):
    def __init__(self, config_filename: str):
        ...
        self._tool_registry.register_custom_tools(self._config.custom_tools)
        self._llm = build_tool_calling_llm(self._config.main, tools_lookup=self._tool_registry.resolve_tool_refs)
    ...
    async def ainvoke(self, input: str, config: Optional[RunnableConfig] = None, **kwargs: Any) -> List[BaseMessage]:
        ...
        return LlmWorker._transform_output(llm_output)
