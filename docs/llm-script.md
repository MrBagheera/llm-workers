...
Tools without `import_tool` or `import_tools` are custom tool definitions. See [Custom Tools](custom-tools.md) for details.
...
For detailed documentation on built-in tools (web fetching, file operations, LLM tools, etc.), see [Built-in Tools](built-in-tools.md).

For documentation on creating custom tools with statement composition, see [Custom Tools](custom-tools.md).
